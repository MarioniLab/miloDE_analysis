...
figures.dir = paste0(root.dir , "figures/da_de/simulations_splatter/controlled/")

# generated simulation data
sces = readRDS(file = paste0(root.dir , "data/processed/da_de/simulations_controlled/sims.Rds"))
simulated_genes = readRDS(file = paste0(root.dir , "data/processed/da_de/simulations_controlled/simulated_genes.Rds"))
stat_da = readRDS(file = paste0(root.dir , "data/processed/da_de/simulations_controlled/stat_da.Rds"))
stat_de = readRDS(file = paste0(root.dir , "data/processed/da_de/simulations_controlled/stat_de.Rds"))

stat_combined = readRDS(file = paste0(root.dir , "data/processed/da_de/simulations_controlled/stat_combined_all_genes.Rds"))
...
get_plot = function(var , gene_type){
  ...
  p = ggplot(current.stat , aes(x = factor(n_markers) , y = factor(n_not_markers) , fill = current.stat[,var])) +
    geom_tile() + 
    facet_wrap(~reducedDim_name + round_sim + order_k , ncol = 12) + 
    scale_fill_viridis(discrete = F , name = "Fraction of DE nhoods" , option = "inferno") +
    theme_bw() + 
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
    labs(x = "# of altered genes contributing to joint variance" , y = "# of altered genes contributing to case variance") +
    theme(legend.position = "top")
...
get_all_plots = function(round_sim , red_dim){
  ...
  p = ggarrange(plotlist = plots , nrow = 11 , ncol = 11 , common.legend = T)
  return(p)
}
...
plots = lapply(1:nrow(anno) , function(i){
  ...
  ggsave(filename = paste0(figures.dir, "umaps/" , as.character(anno$red_dim[i]), "/" , "all_" , as.character(anno$round_sim[i]), ".png"), plot = p, width = 22, height = 24)
})
