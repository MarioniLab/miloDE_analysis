...
get_stat_per_sim_and_nhood_assignment = function(id_sim , id_nhood_assignment , reducedDim_name , spatialFDR_DA.thresh = 0.05 , spatialFDR_DE.thresh = 0.1){
...
stat = merge(stat , unique(stat_da[, c("id_sim" , "effect_size" , "n_not_hvgs", "round_sim" , "order" , "k" , "seed" , "id_nhood_assignment" , "reducedDim_name")]) , by = c("id_sim" , "id_nhood_assignment" , "reducedDim_name") , all.x = T , all.y = F)
...
p = ggplot(stat , aes(x = factor(effect_size) , y = factor(n_not_hvgs) , fill = frac_hoods_DA)) +
  geom_tile() + 
  facet_wrap(~reducedDim_name + round_sim) + 
  scale_fill_viridis(discrete = F , name = "Fraction\nof DA nhoods") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + 
  labs(x = "Effect size for joint variance genes" , y = "# of altered genes\ncontributing to case variance") 
p
ggsave(filename = paste0(figures.dir, "da_stat", ".png"), plot = p, width = 8, height = 6)
...
ids = c( c(1,4,7,10,13,16,19,22,25,28) , c(1,4,7,10,13,16,19,22,25,28) + 1 , c(1,4,7,10,13,16,19,22,25,28)  + 2)
...
p = ggarrange(plotlist = plots , nrow = 3 , ncol = 10 , common.legend = T)
p
ggsave(filename = paste0(figures.dir, "umap_movement", ".png"), plot = p, width = 21, height = 10)
