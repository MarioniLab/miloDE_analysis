...
# load
louvain_clusters = readRDS(paste0(root.dir , "data/processed/lung_macrophages/louvain_clusters_per_hood" ,".Rds"))
...
p = ggplot() +
  ...
  geom_jitter(data = df , aes(x = factor(cluster_id) , y = logFC_bulk , color = bulk_sig_pval) , width = .25 , size = .75) +
  theme_bw() + 
  scale_color_manual(values = cols_sig , name = "Significant \nin bulk DE") + 
  labs(x = "Gene set" , y = "logFC across all macrophages") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
p
ggsave(filename = paste0(figures.dir, "de_per_hood/", "lfc_bulk_boxplot" ,".png"), plot = p, width = 4, height = 3)
  

p = ggplot(df , aes(x = logFC_sig , y = logFC_bulk , col = bulk_sig_pval)) + 
  geom_point(size = .75) + 
  theme_bw() + 
...
p
ggsave(filename = paste0(figures.dir, "de_per_hood/", "lfc_bulk_scater" ,".png"), plot = p, width = 5, height = 4)
