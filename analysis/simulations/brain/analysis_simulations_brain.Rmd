...
milo_de_stat = readRDS(paste0(root.dir , "data/processed/simulations/brain/milo_de_stat__recalculated_embedding.Rds"))
...
p = ggplot(bulk_de_stat_combined[bulk_de_stat_combined$logFC_bulk_clusters < 4 & bulk_de_stat_combined$logFC_bulk_clusters > 0, ] , aes(x = factor(prop) , y = factor(logFC_bulk_clusters_round) , fill = frac_genes_sig)) + 
  geom_tile() + 
...
plots = lapply(unique(milo_de_stat_combined$ct_purity.thresh) , function(ct_purity){
  current.stat = milo_de_stat_combined[milo_de_stat_combined$logFC_bulk_clusters_round < 4 & milo_de_stat_combined$logFC_bulk_clusters_round > 0 & milo_de_stat_combined$ct_purity.thresh == ct_purity, ]
