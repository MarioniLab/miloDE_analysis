...
add_stat = function(sce_milo , clusters){
  ...
  stat_da$cluster_purity = sapply(1:ncol(nhoods_sce) , function(i){
    ...
    out = mean(current.cells %in% sce_milo$cell[sce_milo$cluster %in% clusters])
    return(out)
  })
...
stat = lapply(1:length(sces_milo) , function(j){
  ...
  stat_per_sce = lapply(1:nrow(anno) , function(i){
    ...
    milo_stat = add_stat(current.sce , cluster_splits[[j]])
...
saveRDS(stat , file = paste0(root.dir , "data/processed/simulations/brain/milo_de_stat.Rds"))
