...
anno = expand.grid(order = 2, k = seq(15,30,5), prop = c(0.1), round = c(1:5))
...
sces_milo = lapply(sces , function(sce){
  sce_milo = lapply(1:nrow(anno) , function(i){
    ...
    out = miloDE::assign_neighbourhoods(sce , k = anno$k[i], prop = anno$prop[i] , order = anno$order[i] , reducedDim_name = "scvi_supervised_wt" , filtering = T)
    return(out)
  })
  return(sce_milo)
})
...
saveRDS(stat , file = paste0(root.dir , "data/processed/simulations/all_celltypes/milo_de_stat.Rds"))
