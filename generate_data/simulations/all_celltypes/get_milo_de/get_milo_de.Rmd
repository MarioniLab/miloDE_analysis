...
add_stat = function(sce_milo , celltypes){
  stat_da = add_da(sce_milo , reducedDim_name = "scvi_supervised_wt" , sample_id = "sample", condition_id = "type", col_annotate = NULL)
  ...
  # get celltype purity
  celltype_purity = bplapply(1:ncol(nhoods_sce) , function(i){
    current.cells = which(nhoods_sce[,i] == 1)
    current.cells = rownames(nhoods_sce)[current.cells]
    out = mean(current.cells %in% sce_milo$cell[sce_milo$celltype %in% celltypes])
    return(out)
  }, BPPARAM = mcparam)
  stat_da$celltype_purity = unlist(celltype_purity)
  return(stat_da)
}
...
```{r milo-de-original, message = FALSE}
...
stat = lapply(1:length(sces_milo) , function(j){
  ...
  stat_per_sce = lapply(1:nrow(anno) , function(i){
    ...
    milo_stat = add_stat(current.sce , celltypes[[j]])
...
saveRDS(stat , file = paste0(root.dir , "data/processed/simulations/all_celltypes/milo_de_stat__original_embedding.Rds"))
...
## Recalculated embedding

```{r milo-de-recalc, message = FALSE}
...
saveRDS(stat , file = paste0(root.dir , "data/processed/simulations/all_celltypes/milo_de_stat__recalculated_embedding.Rds"))
