...
get_candidate_genes = function(genes , avg_expr.grid, n_genes = 5){
  candidate_genes = lapply(avg_expr.grid , function(avg_expr){
    current.rowdata = rowdata[rowdata$ENSEMBL %in% genes , ]
    current.genes = sample(current.rowdata$ENSEMBL[current.rowdata$avg_expr_ct == avg_expr] , min(n_genes, length(current.rowdata$ENSEMBL[current.rowdata$avg_expr_ct == avg_expr])))
    current.genes = current.rowdata[current.rowdata$ENSEMBL %in% current.genes , c("ENSEMBL" , "avg_expr_ct")]
...
avg_expr.grid = c(0 , 0.1 , 0.2 , 0.3 , 0.4 , 0.6)


genes_2_simulate = get_candidate_genes(no_de_genes$ENSEMBL , avg_expr.grid = avg_expr.grid , n_genes = 20)
...
saveRDS(genes_2_simulate , file = paste0(root.dir , "data/processed/simulations/simulated_genes.Rds"))
...
for (i in 1:nrow(genes_2_simulate)){
  print(i)
  sce_simulated = add_simulated_counts(sce_simulated, sub_celltype = sub_ct, 
...
saveRDS(sce_simulated , file = paste0(root.dir , "data/processed/simulations/sce_chimera_w_simulations.Rds"))
