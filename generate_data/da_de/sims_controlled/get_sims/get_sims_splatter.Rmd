...
rowdata$avg_group_1 = rowMedians(as.matrix(counts(sim[,sim$Group == "Group1"])))
rowdata$avg_group_2 = rowMedians(as.matrix(counts(sim[,sim$Group == "Group2"])))
...
n_genes.grid = sapply(c(0 , 0.05 , 0.1 , 0.15 , 0.2 , 0.25 , 0.3 , 0.4 , 0.5 , 0.75 , 1) , function(i) return(round(length(genes_hvg) * i)))
...
names(sces) = anno$id
saveRDS(sces , file = paste0(root.dir , "data/processed/da_de/simulations_controlled/sims.Rds"))
...
names(genes_2_simulate) = anno$id
saveRDS(genes_2_simulate , file = paste0(root.dir , "data/processed/da_de/simulations_controlled/simulated_genes.Rds"))
