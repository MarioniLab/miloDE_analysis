...
# discard Hba,Hbb - pulling too much of the signal
rowdata = as.data.frame(rowData(sce))
idx = which(gdata::startsWith(rowdata$SYMBOL , "Hba") | gdata::startsWith(rowdata$SYMBOL , "Hbb") | gdata::startsWith(rowdata$SYMBOL , "mt"))
...
# to ensure baseline of no DA - we will only work with one type, and split cells within this type into 2 synthetic types
#sce = sce[, sce$type == "wt"]
...
markers = markers[markers$FDR < 0.01 & markers$summary.logFC < -.5, c("p.value" , "FDR" , "summary.logFC")]
...
# add average difference in counts per marker
markers = lapply(markers , function(gene){
  out = data.frame(ENSEMBL = gene , 
                   avg_counts_erythroid2 = mean(counts(sce[gene , sce$celltype == "Erythroid2"])) , 
                   avg_counts_erythroid3 = mean(counts(sce[gene , sce$celltype == "Erythroid3"])))
  return(out)
})
markers = do.call(rbind , markers)
markers$diff_counts = markers$avg_counts_erythroid3 - markers$avg_counts_erythroid2
markers = merge(markers , rowdata , all.x = T) 
...
no_de_genes = lapply(unique(sce$celltype) , function(celltype){
  out = get_no_de_genes(celltype , pval.thresh = 0.8) 
  return(out)
})
...
anno = expand.grid(round = c(1:3) , n_markers = seq(0,12,3) , n_not_markers = seq(0,20,5))
...
genes_2_simulate = lapply(1:nrow(anno) , function(i){
  out = data.frame(ENSEMBL = c(sample(markers$ENSEMBL , anno$n_markers[i]) , sample(candidate_no_de_genes , anno$n_not_markers[i])))
  out$gene_type = c(rep("marker" , anno$n_markers[i]) , rep("not_marker" , anno$n_not_markers[i]))
  out$range = sample(c(5), nrow(out) , replace = T)
...
add_batch_corrected_pca = function(sce, genes , reduced.dim_name = "pca.corrected" , batch = "sample" , d = 5, bpparam = NULL , correct = T){
  require(batchelor)
  set.seed(32)
...
add_simulated_counts <- function(sce, gene, frac_cells_change, range){
  ...
  idx_2_change = which(sce$type == "cond_B" & sce$celltype %in% c("Erythroid2"))
...
sces = bplapply(genes_2_simulate , function(current.genes_2_simulate){
  ...
  sce_simulated = add_batch_corrected_pca(sce_simulated, genes = hvgs, reduced.dim_name = "pca.simulations" , batch = "sample" , d = 5, bpparam = NULL , correct = T)
...
# Update gene selections with estimate logFC

```{r gene-sel-upd, message = FALSE}
...
genes_2_simulate = bplapply(1:length(genes_2_simulate) , function(i){
  ...
  de_stat = get_de_stat(sces[[i]] , genes = current.genes$ENSEMBL)
