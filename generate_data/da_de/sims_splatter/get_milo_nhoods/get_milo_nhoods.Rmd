...
stat = bplapply(sims , function(sim){
  stat_per_sim = lapply(1:nrow(anno) , function(i){
    ...
    out = list("all" = nhoods(out_all) , "hvgs" = nhoods(out_hvgs))
    return(out)
  })
  names(stat_per_sim) = anno$id_nhood_assignment
  return(stat_per_sim)
} , BPPARAM = mcparam)
names(stat) = names(sims)
saveRDS(stat , file = paste0(root.dir , "data/processed/da_de/simulations_splatter/nhoods_sims.Rds"))
