...
rowdata$diff = rowdata$avg_group_2 - rowdata$avg_group_1

rowdata$gene_change = !rowdata$DEFacGroup2 == 1

p = ggplot(rowdata , aes(x = factor(gene_change) , y = diff)) + 
  geom_boxplot()
p


# only keep the ones with difference higher than 1
genes_to_keep = rowdata$Gene[rowdata$DEFacGroup2 == 1 | rowdata$diff > 0.5]
sim = sim[genes_to_keep , ]

# re-do rowdata
rowdata = as.data.frame(rowData(sim))
rowdata = rowdata[, c("Gene" , "BaseGeneMean" , "DEFacGroup2")]
rowdata$avg_group_1 = rowMeans(counts(sim[,sim$Group == "Group1"]))
rowdata$avg_group_2 = rowMeans(counts(sim[,sim$Group == "Group2"]))
rowdata$diff = rowdata$avg_group_2 - rowdata$avg_group_1
...
sim = add_batch_corrected_pca(sim, genes = genes_hvg, reduced.dim_name = "pca.hvgs" , batch = "sample" , d = 10)
sim = add_batch_corrected_pca(sim, genes = rowdata$Gene, reduced.dim_name = "pca.all" , batch = "sample" , d = 10)
umaps = as.data.frame( scater::calculateUMAP(t(reducedDim(sim , "pca.hvgs"))) )
reducedDim(sim , paste0("umap_" , "pca.hvgs")) = umaps
umaps = as.data.frame( scater::calculateUMAP(t(reducedDim(sim , "pca.all"))) )
reducedDim(sim , paste0("umap_" , "pca.all")) = umaps


get_plot = function(red_dim){
  umaps = cbind(as.data.frame(colData(sim)) , as.data.frame(reducedDim(sim , red_dim)))
  p = ggplot(umaps , aes(x = V1 , y = V2 , col = Group)) +
    geom_point() +
    theme_bw() + 
    ggtitle(red_dim)
  return(p)
...
p = get_plot("umap_pca.all")
p
p = get_plot("umap_pca.hvgs")
p

```

## Check the difference in expression
...
genes_2_simulate = lapply(rowdata$Gene[!rowdata$DEFacGroup2 == 1] , function(gene){
  out = data.frame(Gene = gene)
  out = merge(out , rowdata , all.x = T , all.y = F)
  out$range = floor(out$avg_group_1 * (out$DEFacGroup2 - 1))
  return(out)
})
...
for (i in 1:nrow(genes_2_simulate)){
  ...
  sce_simulated = add_simulated_counts(sce_simulated, 
                             gene = genes_2_simulate$Gene[i], 
                             range = genes_2_simulate$range[i] )
}
...
umaps = as.data.frame( scater::calculateUMAP(t(reducedDim(sce_simulated , "pca.hvgs"))) )
reducedDim(sce_simulated , paste0("umap_" , "pca.hvgs")) = umaps
umaps = as.data.frame( scater::calculateUMAP(t(reducedDim(sce_simulated , "pca.all"))) )
reducedDim(sce_simulated , paste0("umap_" , "pca.all")) = umaps


get_plot = function(sim , red_dim){
  umaps = cbind(as.data.frame(colData(sim)) , as.data.frame(reducedDim(sim , red_dim)))
  p = ggplot(umaps , aes(x = V1 , y = V2 , col = Group)) +
    geom_point() +
    theme_bw() + 
    facet_wrap(~type + Group) +
    ggtitle(red_dim)
  return(p)
}

p = get_plot(sce_simulated , "umap_pca.all")
p
p = get_plot(sce_simulated , "umap_pca.hvgs")
p
...
- how many not DE genes we change

- how many DE genes we change
...
n_genes.grid = sapply(c(0 , 0.025 , 0.05 , 0.1 , 0.15 , 0.2 , 0.25 , 0.3 , 0.4 , 0.5 , 0.75 , 1) , function(i) return(round(length(genes_hvg) * i)))

anno = expand.grid(round = c(1:3) , n_markers = n_genes.grid , n_not_markers = n_genes.grid)
anno$id = paste0(anno$round , "_n_markers_" , anno$n_markers , "__n_not_markers_" , anno$n_not_markers)

genes_2_simulate = lapply(1:nrow(anno) , function(i){
  out = data.frame(Gene = c(sample(rowdata$Gene[!rowdata$DEFacGroup2 == 1] , anno$n_markers[i]) , sample(rowdata$Gene[rowdata$DEFacGroup2 == 1] , anno$n_not_markers[i])))
  out$gene_type = c(rep("marker" , anno$n_markers[i]) , rep("not_marker" , anno$n_not_markers[i]))
  out = merge(out , rowdata , all.x = T , all.y = F)
  out$range = ceiling(out$avg_group_1 * (out$DEFacGroup2 - 1))
  out$range[out$gene_type == "not_marker"] = ceiling(out$avg_group_1[out$gene_type == "not_marker"]/2)
  return(out)
})
...
sces = bplapply(genes_2_simulate , function(current.genes_2_simulate){
  ...
  sce_simulated = scuttle::logNormCounts(sce_simulated)
  # add re-computed embedding
  sce_simulated = add_batch_corrected_pca(sce_simulated, genes = genes_hvg, reduced.dim_name = "pca.hvgs" , batch = "sample" , d = 10)
  sce_simulated = add_batch_corrected_pca(sce_simulated, genes = rowdata$Gene, reduced.dim_name = "pca.all" , batch = "sample" , d = 10)
  return(sce_simulated)
}, BPPARAM = mcparam)
names(sces) = anno$id
saveRDS(sces , file = paste0(root.dir , "data/processed/da_de/simulations_splatter/sims.Rds"))
...
names(genes_2_simulate) = anno$id
saveRDS(genes_2_simulate , file = paste0(root.dir , "data/processed/da_de/simulations_splatter/simulated_genes.Rds"))
