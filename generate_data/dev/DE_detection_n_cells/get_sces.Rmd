...
get_detection_stat = function(category , response , response.thresh = 0.1){
  ...
  fdr = fp/(fp + tp)
  fomr = fn/(fn + tn)
  out = data.frame(auc = auc , sensitivity = sensitivity , specificity = specificity , fdr = fdr , fomr = fomr)
  return(out)
}
...
get_de_stat = function(de , de_ref = de_bulk_main, rowdata , PValue.thresh = 0.05){
  ...
  out_all = get_detection_stat(de$gene_de_sim , de$PValue , response.thresh = PValue.thresh)
  colnames(out_all) = paste0(c("auc", "sensitivity" , "specificity" , "fdr" , "fomr") , "_all")
...
anno = expand.grid(n_samples_ref = c(2,6,10) , 
                   n_samples_query = c(2,6,10) , sim.idx = c(1:length(sims_main)))
